...
recurCheckbox.addEventListener("change",()=>{ document.getElementById("recurring").style.display=recurCheckbox.checked?"block":"none"; });
...
async function previousMonth(){ currentMonth--; if(currentMonth<0){currentMonth=11; currentYear--;} await loadEventsFromBackend(currentMonth,currentYear); showCalendar(currentMonth,currentYear); displayReminders(); }
async function nextMonth(){ currentMonth++; if(currentMonth>11){currentMonth=0; currentYear++;} await loadEventsFromBackend(currentMonth,currentYear); showCalendar(currentMonth,currentYear); displayReminders(); }
async function jumpToMonthYear(){ const monthSelect=document.getElementById("month"); const yearSelect=document.getElementById("year"); currentMonth=parseInt(monthSelect.value); currentYear=parseInt(yearSelect.value); await loadEventsFromBackend(currentMonth,currentYear); showCalendar(currentMonth,currentYear); displayReminders(); }

window.previousMonth=previousMonth;
window.nextMonth=nextMonth;
window.jumpToMonthYear=jumpToMonthYear;
