...
function addEvent() {
	...
	let eType = eventTypeInput.value;
...
function toggleDiv() {
    const dropdown = document.getElementById("walkInWelcome");
...
function toggleTitleDiv(){
	...
	const walkInDiv = document.getElementById('walkInWelcome');
	if (dropdown.value === "reservedPaid") {
        titleDiv.style.display = "none";
        walkInDiv.style.display = "none";
		eventTitleInput.value = "";
		eventOtherDiv.style.display = "none";
        document.getElementById('eventType').value = "";
		// show the rules and make sure signed
...
function displayReminders() {
	...
	for (let i = 0; i < events.length; i++) {
		...
		if (eventDate.getMonth() ===
			...
			${eventDate.toLocaleDateString()} is 
			${event.eType}`;
...
createYear = generate_year_range(2020, 2075);
